...
class TeacherClient(client: OkHttpClient,moshi: Moshi) : BaseClient(client, moshi) {
    ...
    fun fetchStudentGrades(credential: Credential, student: Student, callback: Callback): Call {
        return client.newCall(fetchGradesRequest(credential, student)).also {
            it.enqueue(callback)
        }
    }
    ...
    fun updateGradesRequest(credential: Credential, studentName: String, grades: List<Int>, exam: Int, callback: Callback): Call {
        return client.newCall(createPatchGradesRequest(credential, studentName, grades, exam))
            .also { call -> call.enqueue(callback) }

    }

    private fun createPatchGradesRequest(credential: Credential, studentName:String, grades: List<Int>, exam: Int): Request {
        val mediaType = "application/json".toMediaType()
        val body = moshi.adapter(GradeUpdateRequest::class.java)
            .toJson(GradeUpdateRequest(grades, exam))
            .toRequestBody(mediaType)

        val url = baseurl.toHttpUrl().resolve("teacher/student/$studentName/grade")!!
        println(url.toString())
        return Request.Builder()
            .url(url)
            .patch(body)
